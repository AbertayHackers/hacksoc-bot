...
class Admin(commands.Cog):
	...
	async def welcome(self, ctx, messageID=None):
		...
		with open("prewrittenText/welcome.txt") as msg:
			if(not messageID):
				await channel.send(msg.read().format(rulesChannel=rulesChannel, committeeRole=committeeRole.mention, constitution=strings["links"]["constitution"], site=strings["links"]["site"]))
			else:
				try:
					message = await channel.fetch_message(messageID)
					await message.edit(content=msg.read().format(rulesChannel=rulesChannel, committeeRole=committeeRole.mention, constitution=strings["links"]["constitution"], site=strings["links"]["site"]))
